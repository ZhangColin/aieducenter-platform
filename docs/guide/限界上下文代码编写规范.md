...
- **prefix**：上下文设计缩写（如 admin → `sys_`）
- **table_name**：蛇形命名（snake_case）

| 上下文 | 前缀 | 示例 |
|--------|------|------|
| admin | `sys_` | `sys_admin_users`, `sys_admin_roles` |
| account | `act_` | `act_users`, `act_profiles` |
| tenant | `ten_` | `ten_tenants`, `ten_members` |
| course | `crs_` | `crs_courses`, `crs_lessons` |
